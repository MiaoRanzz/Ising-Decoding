...
def main(cfg: DictConfig) -> None:
    ...
    if noise_model_mixture_cfg is not None:
        ...
        if code_name.startswith("color"):
            raise ValueError("data.noise_model_mixture is supported only for surface code")
        if noise_model_obj is not None:
            raise ValueError("Configure either data.noise_model or data.noise_model_mixture, not both")
        skip_upscale = bool(getattr(cfg.data, "skip_noise_upscaling", False)) or str(
            os.environ.get("PREDECODER_SKIP_NOISE_UPSCALING", "")
        ).strip().lower() in ("1", "true", "yes", "on")
        mixture_items = OmegaConf.to_container(noise_model_mixture_cfg, resolve=True)
        if not isinstance(mixture_items, list) or not mixture_items:
        ...
        if dist.rank == 0:
            ...
            for task_idx, task in enumerate(noise_model_mixture_train):
                print(
                    ...
                    f"max_group={float(task['upscale_info']['max_group']):.6g}"
                )
    ...
    if use_multi_pairs:
        ...
        if code_name.startswith("color"):
            ...
            if noise_model_mixture_train:
                ...
                task_names = []
                for task_idx, task in enumerate(noise_model_mixture_train):
                    task_common = dict(_gen_torch_common)
                    task_common.update(
                        p_error=task["p_error"],
                        p_min=task["p_error"],
                        p_max=task["p_error"],
                        noise_model=task["train_noise_model"],
                    )
                    task_names.append(task["name"])
                    train_generators.append(QCDataGeneratorTorch(
                        mode="train",
                        verbose=(dist.rank == 0),
                        base_seed=base_seed,
