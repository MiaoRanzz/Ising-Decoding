...
Run ``--phase oracle`` first.  After generating a teacher corpus, run
``--phase teacher --resume <oracle checkpoint>`` to optimize the combined
oracle and endpoint-teacher objective.
"""
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--phase", choices=("oracle", "teacher"), required=True)
    parser.add_argument("--resume", type=Path, default=None, help="Structured checkpoint; required for teacher phase.")
...
def main() -> None:
    ...
    model_cfg, cfg = section(settings, "structured_model"), section(
        settings, "structured_oracle_training" if cli.phase == "oracle" else "structured_teacher_training"
    )
    get = lambda name, default=None: getattr(cli, name) if getattr(cli, name) is not None else cfg.get(name, default)
    ...
    if cli.phase == "teacher" and cli.resume is None:
        raise ValueError("teacher phase requires --resume <oracle/previous structured checkpoint>")
    metadata, _, train_x, train_y = load_corpus(dataset_dir)
    resume = repo_path(cli.resume) if cli.resume else None
    ...
    best = float("inf")
    print(f"[setup] phase={cli.phase} device={device} train={len(train_set)} validation={len(valid_set)} test={len(test_rows)}")
    for epoch in range(1, epochs + 1):
        ...
        payload = {"phase": cli.phase, "epoch": epoch, "base_checkpoint": str(base_checkpoint), "settings": str(settings),
