...
def main():
    args=settings(parse_args());meta=json.loads((args.risk_dataset_dir/'metadata.json').read_text());source=np.load(args.risk_dataset_dir/'source_indices.npy',mmap_mode='r');logits=np.load(args.risk_dataset_dir/'proposal_logits.npy',mmap_mode='r');packed=np.load(args.risk_dataset_dir/'proposal_actions_packed.npy',mmap_mode='r');shot_ptr=np.load(args.risk_dataset_dir/'shot_group_ptr.npy',mmap_mode='r');gptr=np.load(args.risk_dataset_dir/'group_member_ptr.npy',mmap_mode='r');members=np.load(args.risk_dataset_dir/'group_members.npy',mmap_mode='r');_,detsobs,x,_=load_corpus(Path(meta['source_dataset_dir']));shape=tuple(meta['proposal_action_shape']);_,val,test=split_rows(len(source),args.train_fraction,args.validation_fraction,args.split_seed);device=torch.device(args.device or ('cuda' if torch.cuda.is_available() else 'cpu'));matcher,num_obs=build_matcher(meta);cfg=build_model_cfg(SimpleNamespace(project_config=Path(meta['project_config']),checkpoint=Path(meta['proposal_checkpoint']),model_id=meta['proposal_model_id']),meta);maps=_build_stab_maps(int(meta['distance']),str(meta['code_rotation']));am=BatchActionModel().to(device).eval();pipeline=PreDecoderMemoryEvalModule(am,cfg,maps,device).to(device).eval();val_proposal=unpack(packed,val,shape);test_proposal=unpack(packed,test,shape);val_base=baseline_failures(matcher,np.array(detsobs[source[val]],dtype=np.uint8,copy=True),num_obs);test_base=baseline_failures(matcher,np.array(detsobs[source[test]],dtype=np.uint8,copy=True),num_obs);best=None;candidates=sorted(args.checkpoint_dir.glob(args.checkpoint_glob));print(f'[select] {len(candidates)} checkpoints, {len(val)} val, {len(test)} test')
