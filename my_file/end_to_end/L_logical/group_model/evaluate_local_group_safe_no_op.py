...
def scores_for(model,rows,x,source,logits,shot_ptr,gptr,members,batch,device,label='',progress_every=0):
    ...
    with torch.no_grad():
        ...
        for start in range(0,len(rows),batch):
            ...
            if out.ndim != 1:
                raise ValueError("group gate must output one harmful-risk logit per group")
            score[gids]=out
            index=start//batch+1
...
def apply(actions,rows,scores,threshold,shot_ptr,gptr,members):
    ...
    for local,row in enumerate(rows):
        for group in range(int(shot_ptr[row]),int(shot_ptr[row+1])):
            active+=1
            # High harmful-risk logit means applying this group is a veto
            # candidate. Low-confidence groups are retained by default.
